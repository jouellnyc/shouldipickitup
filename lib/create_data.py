...
def lookup_closest_craigs_url_given_zip_with_known_link(zip, city_state_zips):
    ...
    try:
        closest = city_state_zips[min(
            city_state_zips.keys(), key=lambda k: abs(int(k)-int(zip)))]
        return closest
...
def generate_documents_to_import_to_mongodb(zips2knowncityurls, gov_city_state_zips):
    ...
    for zip in (gov_city_state_zips.keys()):
        ...
        try:
            ...
            url = lookup_closest_craigs_url_given_zip_with_known_link(
...
if __name__ == "__main__":
    ...
    try:
        # Round 1
        gov_city_state_zips = create_zips_city_state_dict_from_local_file(
            zip_code_file)
        craigs_city_links = create_craigs_url_dict_from_local_file(
            craigs_links_file)
        zips2knowncityurls = create_map_of_zips_to_known_city_urls(
