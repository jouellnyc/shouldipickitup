...
{'craigs_list_url': 'https://zanesville.craigslist.org',
 ...
 'AltCities': []}
    ...
    for k, v in master_mongo_city_state_zip_map.items():
        mongodoc = f"{{'craigs_list_url' : {k} ,  {v} }}"
        print(mongodoc)
        master_mongo_city_state_zip_data.append(mongodoc)
        #print(f"{{'craigs_list_url':'{k}',", v)
...
if __name__ == "__main__":
    ...
    try:
        ...
        master_mongo_city_state_zip_map = generate_master_documents_import_to_mongodb(craigs_city_links, gov_city_state_zips, mean_zip2craigs_url)
        #mongodb.init_load_city_state_zip_map(master_mongo_city_state_zip_map)
    except Exception as e:
        print("Error: ", e)
